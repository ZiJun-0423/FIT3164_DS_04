...
from backend.db_access.db_stats import db_get_all_stats, db_get_stats_by_team_id, db_get_stats_by_match, db_get_stats_by_match_and_team
...
def get_stats_by_match_id(match_id):
    ...
    stats = db_get_stats_by_match(match_id)
    if stats:
        return jsonify([{"match_id": stat.match_id, "team_id": stat.team_id} for stat in stats]), 200
    else:
...
def get_stats_by_team_id_and_match_id(team_id, match_id):
    ...
    if stats:
        return jsonify([{"match_id": stats.match_id, "team_id": stats.team_id} for stat in stats]), 200
    else:
